...
const Reception: React.FC = () => {
  const floorAccess = useSelector(
    (state: any) => state.floorAccess
  );
  const currentRole = useSelector((state:any) => state.rolereducer);
  const { getFloorByIndex } = useBuildingData();
  const dispatch = useDispatch();

  const handleChangeAccess = (index: number) => {
    dispatch({type:"floorAccess/changeAccess", payload:{index}});
  };

  const handleSetRole = (index: number) => {
    dispatch({type:"role/setRole", payload:{index}});
